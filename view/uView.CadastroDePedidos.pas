...
begin
  if not DataModule1.FDQProdutos.IsEmpty then
    edtPrecoUnitario.Text := FormatFloat('0.00', DataModule1.FDQProdutos.FieldByName('PRECOVENDA').AsFloat);
end;

procedure TFrmPedidoCadastro.AtualizarTotalPedido;
var
  Total: Double;
begin
  Total := 0;
  mtbItensPedido.DisableControls;
  try
    mtbItensPedido.First;
    while not mtbItensPedido.Eof do
    begin
      Total := Total + mtbItensPedido.FieldByName('TotalItem').AsFloat;
      mtbItensPedido.Next;
    end;
  finally
    mtbItensPedido.EnableControls;
  end;

  lblTotalPedido.Caption := Format('Total: R$ %.2f', [Total]);
